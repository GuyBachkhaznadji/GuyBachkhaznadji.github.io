...
# end
#
# def get_job_spec( accounts )
#    holder_names = []
#    for names in accounts
#     holder_names.push(names[:holder_name])
#    end
#    return holder_names
# end
...
#     if search[:title] == job_title
#       # return role[:type]
#       matched_job_title.pop
#       matched_job_title.push(search[:type])
#     end
#   end
#   return matched_job_title[0]
    # search_item= nil
    # test.each do |item|
    #   if item.include? job_title
    #     job_title = item
    #   end
    # end

  # roles.each_value {|name, job_name| job_name == job_title, matched_job_title =  }
  #   matched_job_title = job_name["name1"]
  # else matched_job_title = "No Matches"
  # end
  # return matched_job_title

  #   if role["name1"] == job_title || role["name2"] == job_title || role["name3"] == job_title
  #     matched_job_title = role["name1"]
  #   else
  #
  #   end
  # end
...
def match_key_words(job_title, job_spec, searches)
 ...
 # h[keys[2]] # "val3"
 for search in searches
  #  key_word_values = search[:key_words].values
  #  key_word_keys = search[:key_words].keys
  #  search[:key_words].values[0]
...
def write_search(job_title, job_spec, searches)
  ...
  # written_search = written_search.flatten
  # written_search = [written_search.join(" AND ")]
  ...
  # for search in written_search_collect
  #   if search.include? "OR"
  #     written_search_collect2 << search.push(")")
  #   end
  # end
